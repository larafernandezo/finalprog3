  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <FlatList data={this.state.data.commentarios} keyExtractor={(post) => post.createdAt.toString()} renderItem={({ item }) => (
        <Text> {" "} {item.author}: {item.commentText} </Text>)}/>
        <TextInput keyboardType='default' placeholder='Escribí tu comentario' onChangeText={(text) => { this.setState({ comentario: text }) }} value={this.state.comment} style={styles.input}/>
