...
class MyCamera extends Component {
    ...
    savePhoto() {
        fetch(this.state.photo)
            .then(res => res.blob())//blob amiga BLOB diosa amiga
            .then(image => {
                const refSt = storage.ref(`photos/${Date.now()}.jpg`)
                refStorage.put(image) //ref es un atributo, es una rederencia a esa camara apara despues poder pedirle a esa camara
                    .then(() => {
                        ref.getDownloadURL()
                            .then(url => {
                                console.log(url)
